...
datafile = str('%slos_regrid/los_50Mpc_n%d_z%.3f_fX%.1f_xHI%.2f_dv%d_file%d.dat' % (path,Nlos,z_name,-2,0.25,dvH,0))
...
for l in range(len(xHI_mean)*0+2):
  ...
  for i in range(nfiles):
    ...
    datafile = str('%stau/tau_50Mpc_n%d_z%.3f_fX%.1f_xHI%.2f_dv%d_file%d.dat' %(path,Nlos,z_name,logfX[l],xHI_mean[l],dvH,i))
    data = np.fromfile(str(datafile),dtype=np.float32)
  ...
  array.astype('float32').tofile('1DPS_dimensionless/1DPS_signalandnoise/power_spectrum_signal_21cmFAST_50Mpc_z%.1f_fX%.2f_xHI%.2f_%s_%dkHz_t%dh_Smin%.1fmJy_alphaR%.2f_%dLOS.dat' % (z,logfX[l],xHI_mean[l],telescope,spec_res,t_int,S_min_QSO,alpha_R,n_los),sep='')
