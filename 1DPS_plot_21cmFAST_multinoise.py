...
ax0.legend(frameon=False,loc='upper left',fontsize=fsize,ncol=2)
...
for j in range(0,len(S_min_QSO)):
  ...
  ax0.text(k_bins_cent[3],0.6*PS_noise_med[j,3],r'$\mathrm{%s,}\, t_{\mathrm{int}}=%d\mathrm{hr}$' % (telescope[j],t_int[j]),rotation=20,fontsize=fsize-4)
...
plt.savefig('1DPS_plots/kP21_21cmFAST_multinoise_200Mpc_z%.1f_fX%.2f_xHI%.2f_%dkHz.png' % (z_name,fX_name,xHI_mean,spec_res))
plt.show()
