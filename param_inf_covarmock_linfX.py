...
for i in range(len(files_to_remove)):
   files.remove(files_to_remove[i])
files_to_remove = glob.glob(path_LOS+'*xHI0.6*.dat')
for i in range(len(files_to_remove)):
   files.remove(files_to_remove[i])
files_to_remove = glob.glob(path_LOS+'*xHI0.59*.dat')
for i in range(len(files_to_remove)):
   files.remove(files_to_remove[i])
'''
'''
...
inter_fun_PS21 = interpolate.LinearNDInterpolator(np.transpose([xHI_mean,fX]),PS_sim_mean)
...
par_spc=np.array([xHI_mean,fX])
param_label = ['<xHI>', 'fX']

#Define prior distribution as uniform within the range of our simulated data
fX_cut_min = np.power(10.,-0.6)
slope_cut = (0.81-0.99)/(max_fX-fX_cut_min)
def log_prior(theta):
    xHI_mean1, fX1 = theta
    #if min_xHI <= xHI_mean1 <= max_xHI and min_fX <= fX1 <= max_fX:
    if min_xHI <= xHI_mean1 <= max_xHI and min_fX <= fX1 < fX_cut_min:
      return 0
    elif fX_cut_min <= fX1 <= max_fX and min_xHI <= xHI_mean1 <= 0.99+slope_cut*(fX1-fX_cut_min):
      return 0
    return -np.inf
...
def log_likelihood(theta,signal_mock_mean,covar_mat):
  xHI_mean1, fX1 = theta 
  d_mat = signal_mock_mean-inter_fun_PS21(xHI_mean1, fX1)
...
initial = np.array([0.3,0.007])# + 0.1 * np.random.randn(2)
#soln = minimize(log_likelihood, initial, args=(PS_mock_mean,Mcovar),bounds=([min_xHI,max_xHI],[min_fX,max_fX]))
#print(soln.x)
#para0 = soln.x+1e-4*np.random.randn(n_walk, ndim)
para0 = np.array([0.3,0.007])+1e-4*np.random.randn(n_walk, ndim)
...
labels = [r"$\langle x_{\rm HI}\rangle$",r"$f_{\mathrm{X}}$"]
...
suffix = '200Mpc_xHI%.2f_fX%.1f_%s_%dkHz_Smin%.1fmJy_alphaR%.2f_t%dh_%dsteps_dk%.2f_%dkbins_%dsteps_linfX_wholepspace2.png' % (xHI_mean_mock,logfX_mock,telescope,spec_res,S147,alphaR,tint,Nsteps,d_log_k_bins,len(k_bins_cent),Nsteps)
...
fig=corner.corner(flat_samples,range=[[0.,1.],[0.,0.05]],color='royalblue',smooth=True,labels=labels,label_kwargs=sett
                  ,show_titles=True,title_kwargs=sett,truths=para_mean,truth_color='fuchsia')

fX_mock = np.power(10.,logfX_mock)
corner.overplot_points(fig=fig,xs=[[np.nan,np.nan],[np.nan,np.nan],[xHI_mean_mock,fX_mock],[np.nan,np.nan]],marker='x',markersize=10,markeredgewidth=3,color='darkorange')
